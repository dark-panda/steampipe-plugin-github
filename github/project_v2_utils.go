...
func appendProjectV2ColumnIncludes(m *map[string]interface{}, cols []string) {
	...
	(*m)["includeReadme"] = githubv4.Boolean(slices.Contains(cols, "readme"))
	(*m)["includeResourcePath"] = githubv4.Boolean(slices.Contains(cols, "resource_path"))
	(*m)["includeUrl"] = githubv4.Boolean(slices.Contains(cols, "url"))
	(*m)["includeRepositories"] = githubv4.Boolean(slices.Contains(cols, "repositories") || slices.Contains(cols, "repositories_total_count"))
	(*m)["includeTeams"] = githubv4.Boolean(slices.Contains(cols, "teams") || slices.Contains(cols, "teams_total_count"))
}
...
func projectV2HydrateReadme(_ context.Context, _ *plugin.QueryData, h *plugin.HydrateData) (interface{}, error) {
	project, err := extractProjectV2FromHydrateItem(h)
	if err != nil {
		return nil, err
	}
	return project.Readme, nil
}

func projectV2HydrateResourcePath(_ context.Context, _ *plugin.QueryData, h *plugin.HydrateData) (interface{}, error) {
	project, err := extractProjectV2FromHydrateItem(h)
	if err != nil {
		return nil, err
	}
	return project.ResourcePath, nil
}

func projectV2HydrateUrl(_ context.Context, _ *plugin.QueryData, h *plugin.HydrateData) (interface{}, error) {
	project, err := extractProjectV2FromHydrateItem(h)
	if err != nil {
		return nil, err
	}
...
func projectV2HydrateRepositories(_ context.Context, _ *plugin.QueryData, h *plugin.HydrateData) (interface{}, error) {
	project, err := extractProjectV2FromHydrateItem(h)
	if err != nil {
		return nil, err
	}
	repos := make([]string, 0, len(project.Repositories.Nodes))
	for _, node := range project.Repositories.Nodes {
		repos = append(repos, node.NameWithOwner)
	}
	return repos, nil
}

func projectV2HydrateRepositoriesTotalCount(_ context.Context, _ *plugin.QueryData, h *plugin.HydrateData) (interface{}, error) {
	project, err := extractProjectV2FromHydrateItem(h)
	if err != nil {
		return nil, err
	}
	return project.Repositories.TotalCount, nil
}

func projectV2HydrateTeams(_ context.Context, _ *plugin.QueryData, h *plugin.HydrateData) (interface{}, error) {
	project, err := extractProjectV2FromHydrateItem(h)
	if err != nil {
		return nil, err
	}
	teams := make([]string, 0, len(project.Teams.Nodes))
	for _, node := range project.Teams.Nodes {
		teams = append(teams, node.Slug)
	}
	return teams, nil
}

func projectV2HydrateTeamsTotalCount(_ context.Context, _ *plugin.QueryData, h *plugin.HydrateData) (interface{}, error) {
	project, err := extractProjectV2FromHydrateItem(h)
	if err != nil {
		return nil, err
	}
	return project.Teams.TotalCount, nil
}
